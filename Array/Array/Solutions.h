...
// 和136问题一样可以用异或解决；
// (1^2^...^n再异或给定的数组，则重复两次的全部会消掉，剩下没有出现的数)
// 287. 寻找重复数, 给定范围[1, n]和n+1个数，假设只有一个数重复，找到哪个数出现两次；
// 注：此题不能用异或，因为题设只给定只有一个数重复，而不是只有一个数出现两次；所以需要手算了；
// 排序后比较相邻：时间O(nlogn), 空间O(1)
// 在数组中构建链表，然后利用Floyd判圈法可O(n)时间内得到结果；
// ! 刚好nums[i] in range[1,n], nums.size == n+1, 所以此题的数组刚好是构建好的i -> nums[i]的链表；太厉害了；
// 可视化见官方题解：https://leetcode.cn/problems/find-the-duplicate-number/solution/xun-zhao-zhong-fu-shu-by-leetcode-solution/
