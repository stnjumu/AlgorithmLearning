...
// 141.2. 求环入口; 设链表长L, 非环部分长a, 环长b, 记录第一次相遇slow指针走过的距离为s(已知)；设s=a+X+mb (m>=0, slow也可能在环中走了几圈)，X即入口节点沿前进方向到相遇节点的距离；
// 方法：分析复杂，但实现简单；注：同时求得了非环部分的长度；
// 设Y=b-X，Y即相遇节点沿前进方向到入口节点的距离；
// 由于fast指针是slow指针的2倍速度，则有2s = nb + s (相遇即多走了n圈, n>=1, n>m), s= nb, a+X = nb = (n-1)b+(X+Y), a = (n-1)b+Y; 必有a>=Y
// 所以有 a = Y (mod b)，而a刚好是非环的长度，Y刚好是相遇节点沿前进方向到入口节点的距离, 所以两个指针分别从head和第一次相遇节点出发，以相同速度前进a的距离时，就会刚好相遇在入口节点；
// 而前进a距离前，由于其中一个指针没在环上，另一个在环上，所以必不会相遇，所以两者前进a的距离相遇在入口节点刚好是第一次相遇；
// 141.3. 求环长度；从相遇节点出发，下一次相遇时slow走过的距离刚好是环长度；